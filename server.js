...
// Restrict which hosts this proxy will forward to. The fal client
// hits several different paths under these domains during a realtime
// session (token/auth, websocket upgrade, the model call itself), so
// we allowlist by domain rather than a single narrow model path.
const ALLOWED_HOSTS = ["fal.run", "queue.fal.run", "api.fal.ai", "rest.alpha.fal.ai"];
